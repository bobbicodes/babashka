...
(defn days-in-month
  "Computes the total number of days in a year's month"
  [year month]
  (cond
    (= month 2) (if (leap-year? year) 29 28) ; February
    (some #(= month %) [4 6 9 11]) 30 ; April, June, ...
    :else 31))
...
(defn from
  "Given a year, month and day, compute the moment
   that would be after one Gigasecond has passed."
  [y m d]
